...
namespace Multitudes
{
    ...
    public class Multitudes : BaseUnityPlugin
    {
        ...
        public void Awake()
        {
            ...
            Math.Max(1, (int)(orig(c, origin, chargingRadiusSqr, teamIndex) * (ShouldAffectTeleporterChargeRateConfig.Value ? Multiplier : 1.0f)));
        ...
        private void FixFinalBossZoneFailingToTrigger(ILContext il)
        {
            ...
            {
                c.EmitDelegate<Func<float, float>>(livingPlayerCount => livingPlayerCount / Multiplier);
            }
            else
            {
        ...
        private static float GetLivingPlayerCountHook(Run self) => origLivingPlayerCountGetter(self) * Multiplier;
        private static float GetParticipatingPlayerCountHook(Run self) => origParticipatingPlayerCountGetter(self) * Multiplier;
        ...
        private static void CCSetMultiplier(ConCommandArgs args)
        {
            ...
            if (!float.TryParse(args[0], out var multiplier))
            {
                ...
                Debug.Log($"Multiplier set to {MultiplierConfig.Value}. Good luck! FLOAT");
                SendMultiplierChat();
            }
            else
            {
